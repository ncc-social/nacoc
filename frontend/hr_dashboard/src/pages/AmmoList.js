export function AmmoList() {
  ...
  container.innerHTML = `
    <div class="flex justify-between items-center">
      <h1 class="text-xl font-bold text-gray-900">Ammunition</h1>
      <div class="flex gap-2">
        <input type="text" id="search_input" placeholder="Search ammunition..." class="block w-48 pl-2 pr-2 py-1 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-pelorous-500 focus:border-pelorous-500">
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity on Hand</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Min Stock Level</th>
  ...
  const fetchItems = (query = '', isLoadMore = false) => {
    ...
    fetch(`/api/method/nacoc_armoury.api.search_items?item_type=Ammunition&query=${query}&limit=${limit}&start=${start}`)
      .then(res => res.json())
      .then(data => {
        ...
        if ((!items || items.length === 0) && !isLoadMore) {
          tbody.innerHTML = '<tr><td colspan="5" class="px-3 py-4 text-center text-xs text-gray-500">No ammunition found</td></tr>';
        ...
        const html = items.map(item => `
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-3 py-1.5 whitespace-nowrap text-xs text-gray-500">${item.minimum_stock_level}</td>
            <td class="px-3 py-1.5 whitespace-nowrap text-xs">
              <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${item.status === 'In Stock' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                ${item.status}
              </span>
            </td>
            <td class="px-3 py-1.5 whitespace-nowrap text-xs text-gray-500">
              <a href="#/armoury/details/Ammunition/${item.name}" class="text-pelorous-600 hover:text-pelorous-900 font-medium">View Details</a>
            </td>
          </tr>
        `).join('');
  ...
  const fetchStats = () => {
    fetch('/api/method/nacoc_armoury.api.get_inventory_stats?item_type=Ammunition')
      .then(res => res.json())
      .then(data => {
