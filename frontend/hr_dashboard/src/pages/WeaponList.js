export function WeaponList() {
  ...
  container.innerHTML = `
    <div class="flex justify-between items-center">
      <h1 class="text-xl font-bold text-gray-900">Weapons</h1>
      <div class="flex gap-2">
        <input type="text" id="search_input" placeholder="Search weapons..." class="block w-48 pl-2 pr-2 py-1 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-pelorous-500 focus:border-pelorous-500">
      </div>
    </div>

    <!-- Stats Grid -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4" id="stats_grid">
  ...
  const fetchItems = (query = '', isLoadMore = false) => {
    ...
    fetch(`/api/method/nacoc_armoury.api.search_items?item_type=Weapon&query=${query}&limit=${limit}&start=${start}`)
      .then(res => res.json())
      .then(data => {
        ...
        if ((!items || items.length === 0) && !isLoadMore) {
          tbody.innerHTML = '<tr><td colspan="5" class="px-3 py-4 text-center text-xs text-gray-500">No weapons found</td></tr>';
  ...
  const fetchStats = () => {
    fetch('/api/method/nacoc_armoury.api.get_inventory_stats?item_type=Weapon')
      .then(res => res.json())
      .then(data => {
